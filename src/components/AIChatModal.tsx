...
export const AIChatModal: React.FC = () => {
  ...
  return (
    <>
      {/* Floating Trigger Button for Desktop */}
      {!isOpen && (
        <button
          onClick={() => setIsOpen(true)}
          className="hidden md:flex fixed bottom-6 right-64 z-35 bg-[var(--brand-gold)] text-[var(--brand-primary-dark)] px-3.5 py-2.5 rounded-full shadow-2xl hover:scale-105 transition-all duration-300 items-center gap-2 font-bold font-sans text-xs gold-glow cursor-pointer"
          id="ai-advisor-fab"
        >
          <Sparkles className="w-4 h-4 animate-pulse" />
          <span>AI Botanical Advisor</span>
        </button>
      )}

      {/* Floating Chat Modal */}
      {isOpen && (
        <div className="fixed inset-x-3 bottom-3 sm:inset-auto sm:bottom-6 sm:right-6 z-50 w-auto sm:w-[380px] max-w-[calc(100vw-24px)] bg-[var(--brand-primary-deep)] border border-[var(--brand-gold)]/50 rounded-2xl shadow-2xl overflow-hidden font-sans flex flex-col h-[500px] max-h-[85vh] animate-in slide-in-from-bottom duration-300">
          {/* Header */}
